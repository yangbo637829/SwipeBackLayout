...
/**
 * Created by yangbobo on 17-2-21.
 */
...
public class SystemUtils {
    ...
    private static String getSystemProperty(String key, String defaultValue) {
        Log.d(TAG, "getSystemProperty() called with: key = [" + key + "], defaultValue = [" + defaultValue + "]--------------------------------");
        ...
        try {
            ...
            if (LOGENABLE) {
                Log.d(TAG, "getSystemProperty key = " + key + ", error = " + e.getMessage());
            }
        }

        if (LOGENABLE) {
            Log.d(TAG, "getSystemProperty" + key + " = " + value);
        }
        return value;
    }
        ...
        if (cpuInfo != null && cpuInfo.exists()) {
            ...
            try {
                ...
                if (line != null && line.length() > 0 && line.toLowerCase(Locale.US).contains("arch64")) {
                    if (LOGENABLE) {
                        Log.d(TAG, "###############isCPUInfo64() , " + PROC_CPU_INFO_PATH + " contains is arch64");
                    }
                    return true;
                } else {
                    if (LOGENABLE) {
